...
//implement higher order function XXX
//
...
function getFullNames(runners) {
  const array = [];
  runners.forEach(i => {
    array.push(`${i.last_name}, ${i.first_name}`)
  })
...
function firstNamesAllCaps(runners) {
  ...
  runners.map (i => {
    ...
    array.push(i.first_name.toUpperCase());
 ...
 * Implement this function using f ilter().
 * 
 * @param runners array of runners like the one inside the /data/runners.js file.
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const array = runners.filter(function(run) {
...
function tallyUpDonations(runners) {
  const sum = runners.reduce(function(run, donate) {
    return run + donate.donation;
  }, 0);  
...
let  counterMakerWithLimit = function(maxValue) {
  let count = 0
   function counter () {
    if (count <= maxValue) {
      return count++
